function save_bootstrapped_singleslice_modularity_results(full_data, target_prom, target_stim, window_length, slide, n_boots)
output_name = join([target_prom, "_", target_stim, "_w", num2str(window_length), "_s", num2str(slide), "_singl_nb", num2str(n_boots), ".mat"],'');
    ...
    % get single-slice module assignments and modularity
    [n_cells, n_windows] = size(A, [1 3]);
    S_singl_n_boot = zeros(n_cells, n_windows, n_boots);
    S_singl_t_boot = zeros(n_cells, n_windows, n_boots);
    ...
    for i = 1:n_boots
        ...
        gamma = 1;
        nIter = 100;
        [S_singl_n] = get_reindexed_singleSliceModules(Z_n, gamma, nIter);
        [S_singl_t] = get_reindexed_singleSliceModules(Z_t, gamma, nIter);
        
        % reshape and re-index modules
        S_singl_n = reshape(S_singl_n, [n_undropped, n_windows]);
        S_singl_n_boot(~dropped_rows(:,i),:,i) = reindex_modules(S_singl_n);
        S_singl_t = reshape(S_singl_t, [n_undropped, n_windows]);
        S_singl_t_boot(~dropped_rows(:,i),:,i) = reindex_modules(S_singl_t);
    end
    ts(2) = toc;
    ...
    for i = 1:n_boots
        ...
        S_singl_n_temp = S_singl_n_boot(~dropped_rows(:,i),:,i);
        S_singl_t_temp = S_singl_t_boot(~dropped_rows(:,i),:,i);
    
        % loop through windows and calculate modularity
        for t = 1:n_windows
            [B_n, twom_n] = modularity_w_sign(A_n_temp(:,:,t),gamma);
            Q_ms_n_boot(t,i) = sum(B_n.*(S_singl_n_temp(:,t) == S_singl_n_temp(:,t)'),'all')/twom_n;
    
            [B_t, twom_t] = modularity_w_sign(A_t_temp(:,:,t),gamma);
            Q_ms_t_boot(t,i) = sum(B_t.*(S_singl_t_temp(:,t) == S_singl_t_temp(:,t)'),'all')/twom_t;
        end
    end
    ts(3) = toc;
    ...
    for i = 1:n_boots
        ...
        flexibility_n_boot(~dropped_rows(:,i),i) = get_flexibility(S_singl_n_temp);
        flexibility_t_boot(~dropped_rows(:,i),i) = get_flexibility(S_singl_t_temp);
    end
    ts(4) = toc;
    ...
    for i = 1:n_boots
        ...
        allegiance_n_boot(~dropped_rows(:,i),~dropped_rows(:,i),i) = get_consensus(S_singl_n_temp);
        allegiance_t_boot(~dropped_rows(:,i),~dropped_rows(:,i),i) = get_consensus(S_singl_t_temp);
        ...
        RI_mat_n_boot{i} = get_recruitment_and_integration(S_singl_n_temp, S_conn_n_boot(~dropped_rows(:,i),i));
        RI_mat_t_boot{i} = get_recruitment_and_integration(S_singl_t_temp, S_conn_t_boot(~dropped_rows(:,i),i));
    end
    ts(5) = toc;
    ...
    results = {'folded_IDs', 't_end', 'dropped_rows'...
               'S_singl_n_boot', 'S_singl_t_boot', 'Q_ms_n_boot', 'Q_ms_t_boot', ...
